<style>
	img.photo{
		width:150px;
		float: left;
		margin-right: 15px;
		padding-top:7px;
		border-radius: 50%;
	}
</style>


<h1 class="col-sm-12 title">Page de présentation de notre équipe</h1>
<h2 class="col-sm-12 subtitle">Nous nous sommes présentés d'une manière qui nous représente</h2>

<h4 class="nom">Chef de projet : Loïc Bertrand</h4>
<p class="col-sm-12 paragraph">
	<img src="img/photo/loic.jpg" alt="photo de Loïc" class="photo">
	<br>
	<br>
	Je m'appelle Loïc, j'ai 20 ans. Après une première année à l'Université de Technologie de Belfort-Montbéliard, j'ai décidé de me réorienter vers un DUT informatique.
	J'ai choisi cette nouvelle voie afin d'aborder une approche plus technique de l'informatique.
	Au travers de cette nouvelle formation, je cultive et je mets en pratique des notions qui me passionnent.
</p>

<h4 class="nom">Membre : Quentin Delignou</h4>
<p class="col-sm-12 paragraph" title="Indice: utiliser un traducteur de morse">
	<img src="img/photo/quentin.jpg" alt="photo de Quentin" class="photo">
	<br>
	−··· −−− −· ·−−− −−− ··− ·−· −−··−−  <br>
	·−−− ·  −− ·−−−−· ·− ·−−· ·−−· · ·−·· ·−·· ·  −−·− ··− · −· − ·· −· −−··−−  ·−−− ·−−−−· ·− ··  ··−−− −−−−−  ·− −· ··· ·−·−·− <br />
	·−−− ·−−−−· ·− ·· −− ·  ·−·· · ···  −− ·− −· −−· ·− ··· −−··−−  ·−·· · ···  ·−−− · ··− −··−  ···− ·· −·· ··−·· −−− ···  · −  ·−·· ·−  −· −··· ·− ·−·−·− <br />
	···− −−− ·· ·−·· ·−  ·−−− ·  ··· ·− ·· ···  ·−−− ·− −− ·− ·· ···  −−·− ··− −−− ··  −·· ·· ·−· ·  −·· ·−−−−· ·− ··− − ·−· ·  −−·− ··− ·− −· −··  ·−−− ·  −·· −−− ·· ···  −− ·  ·−−· ·−· ··−·· ··· · −· − · ·−·  ·−·−·− ·−·−·− ·−·−·−
</p>

<h4 class="nom">Membre : Tom Jacob</h4>
<p class="col-sm-12 paragraph">
	<img src="img/photo/tom.jpg" alt="photo de Tom" class="photo">
	<br>
	echo "Bonjour". <br>
	Je me présente, je m'appelle $prenom = Tom , et j'ai actuellement $age = 20 ans. <br>
	Je viens d'une contrée lointaine appelée "Saône-et-Loire". <br>
	Ma plus grande passion dans la vie, c'est surtout passion.musique <br>
	Mais il ne faudrait surtout pas oublier passion.manger. <br>
	<br><br>
	Mon orientation post-bac s'est déroulée de cette manière:<br>
	switch(annee){<br>
		case 2016:<br>
		DUT Génie Electrique<br>
		break;<br>
		<br>
		case 2018:<br>
		DUT Informatique<br>
		break;<br>
	}<br><br>
	Et surtout, n'oubliez pas, dans la vie, <strong><span title="C.f. Miloud Fouzi">TOUT EST FICHIER.</span></strong>
</p>

<h4 class="nom">Membre : Erwan Vinot</h4>
<p class="col-sm-12 paragraph">
	<img src="img/photo/erwan.jpg" alt="photo d'Erwan" class="photo">
	<br>
	Je suis Erwanomax, j'ai 18 ans.
</p>

<div class="row col-md-12">
	<a class="button back" href="page.php">Retour à la page "technique"</a>
</div>
